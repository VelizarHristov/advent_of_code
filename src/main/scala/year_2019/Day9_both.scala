...
def day9(): Unit =
  ...
  val progArray = Source.fromFile("resources/2019/9").getLines.next.split(',').map(BigInt.apply)
  ...
  val res1 = Program(prog.clone()).run(Seq(1)).head
  val res2 = Program(prog.clone()).run(Seq(2)).head
